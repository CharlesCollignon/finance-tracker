...
export function QuickAddSheet({
  ...
  // A half-typed abandoned entry must never reappear attached to a later one.
  useEffect(() => {
    if (!open) {
      return;
    }
    setAmount("");
    setCategoryId("");
    setOccurredOn(defaultDate ?? today);
    setNote("");
    setTagIds([]);
    setQuery("");
            ...
            <View className="mb-4 flex-row flex-wrap gap-2">
              {[
                ...
                return (
                  ...
                  >
                    <Text
                      className={cn("text-sm", active && "text-primary-ink")}
                    >
                      {option.label}
                    </Text>
                  </Pressable>
              ...
              >
                <Text
                  className={cn(
                    "text-sm",
                    showDatePicker && "text-primary-ink",
                  )}
                >
                  Another day
                </Text>
                ...
                <View className="mb-4 flex-row flex-wrap gap-2">
                  {tags.map((tag) => {
                    ...
                    return (
                      ...
                      >
                        <Text
                          className={cn("text-sm", on && "text-primary-ink")}
                        >
                          {tag.name}
                        </Text>
                      </Pressable>
