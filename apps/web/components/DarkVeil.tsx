...
type Props = {
  /**
   * Called when the shader cannot run — no WebGL context, or a program that
   * will not link. Both are ordinary on a software renderer (a VM, a remote
   * desktop, a blocklisted driver) and neither is a reason to take the page
   * down: this component is mounted in the app shell, so an unguarded throw
   * here is an error overlay on every screen in the app.
   */
  onUnavailable?: () => void;
  hueShift?: number;
...
export default function DarkVeil({
  ...
  useEffect(() => {
    ...
    // only surfaces on the first draw, as `undefined.forEach`, from inside
    // the library. Checked here instead, where it can be handled.
    const linked =
    ...
    const loop = () => {
      program.uniforms.uTime.value = still
        ? 0
      ...
      // A still veil is one frame, not sixty a second of the same frame.
      if (!still) {
