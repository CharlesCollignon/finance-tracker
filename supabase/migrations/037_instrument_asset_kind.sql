...
-- `025` sets out why this is not optional: Postgres grants EXECUTE on a new
-- function to PUBLIC, and `anon` is a role behind a key that ships in every
-- browser. `create or replace` above keeps the old signature's grants, but
-- the twelve-argument form is a *new* function as far as Postgres is
-- concerned, so it arrives with PUBLIC's default grant and needs the same
-- treatment as its eleven-argument sibling.
revoke all on function store_instrument_reading(
  uuid, text, numeric, text, jsonb, jsonb, jsonb, numeric, jsonb, text,
  smallint, text
) from public;
